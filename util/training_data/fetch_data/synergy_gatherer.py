...
def get_synergies(threads=200, save_interval=10000, fetched_data_name='testing', number_of_decks=None, dummy_data=3):
    # setup folders
    root_folder_path = data_util.get_data_path('raw_datasets', ['training_data'])
    general_data_folder_structure =     {
                                            'decklist_hashes.json':'[]',
                                            'name_data.json':'{}',
                                            'deck_characteristics.json':{"tags":[], "tribes":[]}
                                        }
    raw_dataset_folder_structure =      {fetched_data_name:{
                                            'final_datasets':{
                                                'binary.json':[],
                                                'synergies.json':{},
                                                'adjusted_binary':[]
    ...
    dir_util.create_folder_structure(os.path.join(root_folder_path, fetched_data_name), raw_dataset_folder_structure, overwrite=True)
    ...
    # saving files
    list_of_files = [('deck_characteristics.json', deck_characteristics),('name_data.json', name_data), ('total_decks.json', total_decks), ('synergies.json', synergies), ('binary_list.json', binary_data)]
    ...
    create_raw_dataset.create_raw_dataset(binary_data, synergies, total_decks, raw_dataset_path, dummy_data, name_data)
